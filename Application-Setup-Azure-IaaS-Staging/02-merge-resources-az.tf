...
module "get_vnet_env-1" {
  ...
  var_next_hop_in_ip_address = module.get_azfw_policy_rule.az_fw_prvt_ip_output
  # depends_on = [ module.get_azfw_policy_rule ]
}
...
# # Calling HUB Vnet configuration here
# module "get_hub_vnet_config" {
#   source = "./General-Networking-Components"
#   vnet_resource_group_location = azurerm_resource_group.tf-rg.location
#   vnet_resource_group_name = azurerm_resource_group.tf-rg.name
#   var_next_hop_in_ip_address = module.get_azfw_policy_rule.az_fw_prvt_ip_output
...
# Calling Azure Firewall and Policy configuration

module "get_azfw_policy_rule" {
  source = "./Az-Firewall"
  var_azfw_rg_location = azurerm_resource_group.tf-rg.location
  var_azfw_rg_name = azurerm_resource_group.tf-rg.name
  var_azfw_subnet = module.get_vnet_env-1.hub_az_fw_sub_output
}


# Calling VPN Gateway configuration

module "get_vpn_gw" {
  source = "./Az-VPN-Gw"
  var_vpn_rg_location = azurerm_resource_group.tf-rg.location
